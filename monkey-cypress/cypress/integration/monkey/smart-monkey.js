...
function typeCharKey(){
    ...
    if(focused){
        ...
        // cy.get('body').type('Hola mundo')
        cy.get('body').type('krakenCypressTest@automatizacion.com.co')
        info = "INVALID. No element is in focus"
    }
        ...
        if($inputs.length > 0){
            ...
            if(!Cypress.dom.isHidden(inp)) {
                ...
                if(inp.getAttribute("type") == "email"){
                    ...
                    // cy.wrap('//input[@id="ember7"]').type('gcastrot82@hotmail.com');
                // cy.wrap(inp).type('gcastrot82@hotmail.com');
                    // cy.get(inp).type('gcastrot82@hotmail.com');
                    // cy.wrap('gcastrot82@hotmail.com').type();
                    ...
                    let type = 'KrakenCypress123456@{enter}'//faker.random.alphaNumeric
                    cy.wrap(inp).type(type);
                    info = `Input ${inp.id} was filled with ${type}`
                }
                else {
